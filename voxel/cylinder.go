...
// BuildCylinder ...
func BuildCylinder(r, h float32) IndicatorFunc {
	return func(x, y, z float32) bool {
		return Cylinder(x, y, z, 0, 0, 0, r, h)
	}
}

//Cylinder ...
//Center is in the point (x0,y0,z0)
func Cylinder(x, y, z, x0, y0, z0, r, h float32) bool {
	xx, yy, zz := x-x0, y-y0, z-z0

	return xx*xx+yy*yy <= r*r && zz <= h/2 && zz >= -h/2
}

func BuildCylinderVM(R, H float32, N uint32) BlenderVoxelFormat {
	const (
		zero = uint32(0)
	)
